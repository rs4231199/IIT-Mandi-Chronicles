...
import { FlatList, StyleSheet, Text, View, Image, TouchableWithoutFeedback, Dimensions, ActivityIndicator, ScrollView, AsyncStorage } from 'react-native';
...
class mainPost extends React.Component {
  ...
  retrieveData = async () => {
    try {
      ...
      let finalDateInMilliseconds1 = date2.getTime();
      let date3 = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 23, 59, 59, 999);
      let finalDateInMilliseconds2 = date3.getTime();
      let initialQuery = await database.collection('Posts')
        .where('miliTime', '<=', finalDateInMilliseconds2)
        .orderBy('miliTime')
        .limit(this.state.limit)
      initialQuery = initialQuery.where('miliTime', '>=', finalDateInMilliseconds1)
  ...
  retrieveMore = async () => {
    try {
      ...
      let additionalQuery = await database.collection('Posts')
        ...
        .limit(this.state.limit)
      additionalQuery = additionalQuery.where('miliTime', '>=', finalDateInMilliseconds1)
      ...
      <View style={{ flex: 1 }}>
        ...
        { 
          ...
          <FlatList
            ...
            ListEmptyComponent={<Text style={{textAlign: 'center', marginBottom: 300 }}>No Events Today</Text>}
...
today.navigationOptions = ({ navigation }) => {
